    ...
    <main className="min-h-screen bg-[var(--bg-primary)]">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 hero-text-reveal">
          <p
            className="text-xs tracking-[0.5em] uppercase mb-6 animate-fade-in stagger-1"
            style={{ fontFamily: "var(--font-mono)", color: "var(--accent-gold)" }}
          >
            Photographe professionnel
          </p>
          <h1
            className="mb-6 animate-fade-in-up stagger-2"
            style={{ fontFamily: "var(--font-display)", color: "var(--text-primary)", fontSize: "clamp(3rem, 8vw, 6rem)", lineHeight: "1.05" }}
          >
            Olivier Reynes
          </h1>
          <p
            className="text-sm md:text-base mb-4 tracking-[0.3em] uppercase animate-fade-in stagger-3"
            style={{ fontFamily: "var(--font-mono)", color: "var(--text-muted)", opacity: 0.7 }}
          >
            Immobilier · Architecture · Mariages
          </p>
          <div
            className="mx-auto mb-10 animate-fade-in stagger-4"
            style={{ width: "60px", height: "2px", background: "linear-gradient(90deg, transparent, var(--accent-gold), transparent)" }}
          />
          <a
            href="/galleries"
            className="btn-gold animate-fade-in stagger-5"
          >
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-2 md:grid-cols-4 gap-8">
          {STATS.map((stat, i) => (
            <div key={i} className="text-center">
              <div className="text-2xl md:text-3xl mb-1" style={{ fontFamily: "var(--font-display)", color: "var(--accent-gold)" }}>
                {stat.value}
              </div>
              <div className="text-[0.65rem] tracking-[0.2em] uppercase" style={{ fontFamily: "var(--font-mono)", color: "var(--text-muted)" }}>
                {stat.label}
              </div>
            </div>
          ))}
      ...
      <section className="py-24 px-6 max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {SERVICES.map((service, i) => (
            ...
            >
              <span className="text-4xl mb-6 block transition-transform duration-500 group-hover:scale-110">{service.icon}</span>
              <h3 className="mb-3" style={{ fontFamily: "var(--font-display)", color: "var(--text-primary)" }}>
                {service.title}
              </h3>
              <p className="text-xs leading-relaxed mb-4" style={{ color: "var(--text-muted)" }}>
                {service.desc}
              </p>
              <span className="text-xs tracking-[0.15em] uppercase transition-colors duration-300" style={{ color: "var(--accent-gold)" }}>
                Découvrir →
              </span>
            </a>
          ))}
            ...
            {TESTIMONIALS.map((t, i) => (
              <div key={i} className="glass card-hover p-8" style={{ borderRadius: "2px" }}>
                <svg className="mb-6" width="32" height="20" viewBox="0 0 32 20" fill="none">
                  <path
                    d="M0 20V12C0 5.373 5.373 0 12 0H14V5.333H12C8.318 5.333 5.333 8.318 5.333 12V20H0ZM18.667 20V12C18.667 5.373 24.039 0 30.667 0V5.333C27.463 5.333 24.889 7.907 24 10.667V20H18.667Z"
      ...
      <section className="py-32 px-6 text-center relative overflow-hidden">
        ...
        <div className="relative max-w-[600px] mx-auto">
          ...
          <h2 className="mb-6" style={{ fontFamily: "var(--font-display)", color: "var(--text-primary)" }}>
            Un projet en tête ?
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a
              href="/galleries"
              className="text-xs tracking-[0.2em] uppercase px-6 py-3 border border-transparent hover:border-[var(--glass-border)] transition-all duration-300"
